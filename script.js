...
let board = [];  
let solvedBoard = []; // Biến lưu giữ đáp án của bảng mới  
...
function startGame() {  
    ...
    solvedBoard = JSON.parse(JSON.stringify(board)); // Lưu bản sao đáp án  
...
function showSolution() {  
    ...
    // Hiển thị đáp án của bảng đã xáo trộn  
    const cells = document.querySelectorAll('.sudoku-cell');  
    for (let i = 0; i < 9; i++) {  
        for (let j = 0; j < 9; j++) {  
            if (board[i][j] === 0) {  
                cells[i * 9 + j].textContent = solvedBoard[i][j]; // Sử dụng solvedBoard  
